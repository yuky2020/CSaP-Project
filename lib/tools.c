...
int32_t hashCode(PackageData tohash)
{
  ...
  {
    ret = ret + ((int)tohash.from[i] + 3 * (int)tohash.to[i]);
  } //33 is the famus magic number
...
int read_PackageData(PackageData *toread, FILE *fp)
{
  ...
  if (fread(&toread->type, sizeof(int), 1, fp) < 0)
  {
    perror("read");
    return 1;
  }
  //read the from user from file
  if (fread(&fromlen, sizeof(int), 1, fp) < 0)
  {
    perror("read");
    return 1;
  }
  if (fread(&toread->from, fromlen + 1, 1, fp) < 0)
  {
    perror("read");
    return 1;
  }
  //read the to user from file
  if (fread(&tolen, sizeof(int), 1, fp) < 0)
  {
    perror("read");
    return 1;
  }
  if (fread(&toread->to, tolen + 1, 1, fp) < 0)
  {
    perror("read");
    return 1;
  }
  //read the size of AudioDataf
  if (fread(&toread->size, sizeof(int), 1, fp) < 0)
  {
    perror("read f");
    return 1;
  }
  //write audioDataf
  if (fread(&toread->message.frameIndex, sizeof(int), 1, fp) < 0)
  {
    perror("read f");
    return 1;
  }
  //all frames are important
  if (fread(&(toread->message.maxFrameIndex), sizeof(int), 1, fp) < 0)
  {
    perror("read f");
    return 1;
  }
  ...
  if (fread(&timelen, sizeof(int), 1, fp) < 0)
  {
    perror("write f");
    return 1;
  }
...
int datecmp(char a[TIMESTAMPS], char b[TIMESTAMPS])
{
  ...
  if ((int)a[22] > (int)b[22])
    return 1; //check millenio
  else if ((int)a[22] < (int)b[22])
    return -1;
  if ((int)a[23] > (int)b[23])
    return 1; //check hundreds of years
  else if ((int)a[23] < (int)b[23])
    return -1;
  if ((int)a[24] > (int)b[24])
    return 1; //check decades
  else if ((int)a[24] < (int)b[24])
    return -1;
  if ((int)a[25] > (int)b[25])
    return 1; //check  years
  else if ((int)a[25] < (int)b[25])
    return -1;
  //now check mounth
